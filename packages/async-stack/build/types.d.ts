export declare type IStackEvent = "pushstart" | "pushend" | "popstart" | "popend" | "replace";
export declare type IStatus = "pushing" | "popping" | "settled";
export declare type ListenerFn<T> = ({ items, }: {
    action: IStackEvent;
    key: string;
    items: IItem<T>[];
    getItemByKey: (key: string) => T | undefined;
}) => void;
export declare type IItem<T> = T & {
    key: string;
    status: IStatus;
};
export declare type IReplaceOptions<T> = T & {
    replaceAmount?: number;
    key?: string;
};
export declare type IPushOptions<T> = T & {
    key?: string;
};
